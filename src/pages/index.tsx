...
export default function Home() {
  ...
  if (loading) return <div className={styles.center} style={{height:'60vh'}}>加载中…</div>;
    ...
    <div className={styles.wrap}>
      ...
      <main style={{padding:18}}>
        ...
        <div className={styles.tiles}>
          <div className={styles.tile} onClick={()=>router.push('/worker')}>
            <h3>Worker（员工作业）</h3>
            <p className={styles.muted}>扫码录入、选择工种，进入相应工作页面。</p>
          </div>

          <div className={cx(styles.tile, !isAdmin && styles.disabled)}
               onClick={()=>{ if(isAdmin) router.push('/admin'); }}>
            <h3>Othermine（管理员）</h3>
            <p className={styles.muted}>查看统计、追溯、全局数据。</p>
            {!isAdmin && <p className={styles.hint}>你没有权限进入此区域</p>}
          </div>
        </div>
      </main>
    </div>
